...
Q_LOGGING_CATEGORY(sortingLog, "calendar.sorting")
...
{
    qCDebug(sortingLog) << "Comparing datetimes:" << dt1 << "allDay:" << isAllDay1 << "with" << dt2 << "allDay:" << isAllDay2;
    ...
    if (isAllDay1 || isAllDay2) {
        ...
        qCDebug(sortingLog) << "Comparison result:" << result;
        return result;
    }
...
{
    qCDebug(sortingLog) << "Comparing event start dates:" << e1->dtStart() << "with" << e2->dtStart();
...
{
    qCDebug(sortingLog) << "Comparing event start dates (more than):" << e1->dtStart() << "with" << e2->dtStart();
    DateTimeComparison res = compare(e1->dtStart(), e1->allDay(), e2->dtStart(), e2->allDay());
    if (res == Equal) {
...
{
    qCDebug(sortingLog) << "Comparing event summaries:" << e1->summary() << "with" << e2->summary();
...
{
    qCDebug(sortingLog) << "Comparing event summaries (more than):" << e1->summary() << "with" << e2->summary();
    return QString::compare(e1->summary(), e2->summary(), Qt::CaseInsensitive) > 0;
}

bool KCalendarCore::Events::endDateLessThan(const Event::Ptr &e1, const Event::Ptr &e2)
{
    qCDebug(sortingLog) << "Comparing event end dates:" << e1->dtEnd() << "with" << e2->dtEnd();
...
{
    qCDebug(sortingLog) << "Comparing event end dates (more than):" << e1->dtEnd() << "with" << e2->dtEnd();
    DateTimeComparison res = compare(e1->dtEnd(), e1->allDay(), e2->dtEnd(), e2->allDay());
    if (res == Equal) {
        return Events::summaryMoreThan(e1, e2);
    } else {
...
{
    qCDebug(sortingLog) << "Comparing journal dates:" << j1->dtStart() << "with" << j2->dtStart();
...
{
    qCDebug(sortingLog) << "Comparing journal dates (more than):" << j1->dtStart() << "with" << j2->dtStart();
    DateTimeComparison res = compare(j1->dtStart(), j1->allDay(), j2->dtStart(), j2->allDay());
    return (res & After || res & AtEnd);
}
...
{
    qCDebug(sortingLog) << "Comparing journal summaries:" << j1->summary() << "with" << j2->summary();
...
{
    qCDebug(sortingLog) << "Comparing journal summaries (more than):" << j1->summary() << "with" << j2->summary();
    return QString::compare(j1->summary(), j2->summary(), Qt::CaseInsensitive) > 0;
}

bool KCalendarCore::Todos::startDateLessThan(const Todo::Ptr &t1, const Todo::Ptr &t2)
{
    qCDebug(sortingLog) << "Comparing todo start dates:" << t1->dtStart() << "with" << t2->dtStart();
...
{
    qCDebug(sortingLog) << "Comparing todo start dates (more than):" << t1->dtStart() << "with" << t2->dtStart();
    DateTimeComparison res = compare(t1->dtStart(), t1->allDay(), t2->dtStart(), t2->allDay());
    if (res == Equal) {
...
{
    qCDebug(sortingLog) << "Comparing todo due dates:" << (t1->hasDueDate() ? t1->dtDue().toString() : "no due date") 
                        << "with" << (t2->hasDueDate() ? t2->dtDue().toString() : "no due date");
    if (!t1->hasDueDate()) {
        return false;
    }
    if (!t2->hasDueDate()) {
...
{
    qCDebug(sortingLog) << "Comparing todo due dates (more than):" << (t1->hasDueDate() ? t1->dtDue().toString() : "no due date")
                        << "with" << (t2->hasDueDate() ? t2->dtDue().toString() : "no due date");
    if (!t2->hasDueDate()) {
        return false;
    }
...
{
    qCDebug(sortingLog) << "Comparing todo priorities:" << t1->priority() << "with" << t2->priority();
...
{
    qCDebug(sortingLog) << "Comparing todo priorities (more than):" << t1->priority() << "with" << t2->priority();
    if (t1->priority() > t2->priority()) {
        return true;
    } else if (t1->priority() == t2->priority()) {
        return Todos::summaryMoreThan(t1, t2);
...
{
    qCDebug(sortingLog) << "Comparing todo completion percentages:" << t1->percentComplete() << "with" << t2->percentComplete();
...
{
    qCDebug(sortingLog) << "Comparing todo completion percentages (more than):" << t1->percentComplete() << "with" << t2->percentComplete();
    if (t1->percentComplete() > t2->percentComplete()) {
        return true;
    } else if (t1->percentComplete() == t2->percentComplete()) {
        return Todos::summaryMoreThan(t1, t2);
...
{
    qCDebug(sortingLog) << "Comparing todo summaries:" << t1->summary() << "with" << t2->summary();
...
{
    qCDebug(sortingLog) << "Comparing todo summaries (more than):" << t1->summary() << "with" << t2->summary();
    return QString::compare(t1->summary(), t2->summary(), Qt::CaseInsensitive) > 0;
}

bool KCalendarCore::Todos::createdLessThan(const Todo::Ptr &t1, const Todo::Ptr &t2)
{
    qCDebug(sortingLog) << "Comparing todo creation dates:" << t1->created() << "with" << t2->created();
...
{
    qCDebug(sortingLog) << "Comparing todo creation dates (more than):" << t1->created() << "with" << t2->created();
    DateTimeComparison res = compare(t1->created(), t1->allDay(), t2->created(), t2->allDay());
    if (res == Equal) {
        return Todos::summaryMoreThan(t1, t2);
    } else {
...
{
    qCDebug(sortingLog) << "Comparing incidence dates:" << i1->dateTime(Incidence::RoleSort) << "with" << i2->dateTime(Incidence::RoleSort);
...
{
    qCDebug(sortingLog) << "Comparing incidence dates (more than):" << i1->dateTime(Incidence::RoleSort) << "with" << i2->dateTime(Incidence::RoleSort);
    DateTimeComparison res = compare(i1->dateTime(Incidence::RoleSort), i1->allDay(), i2->dateTime(Incidence::RoleSort), i2->allDay());
    if (res == Equal) {
...
{
    qCDebug(sortingLog) << "Comparing incidence creation dates:" << i1->created() << "with" << i2->created();
...
{
    qCDebug(sortingLog) << "Comparing incidence creation dates (more than):" << i1->created() << "with" << i2->created();
    DateTimeComparison res = compare(i1->created(), i1->allDay(), i2->created(), i2->allDay());
    if (res == Equal) {
        return Incidences::summaryMoreThan(i1, i2);
    } else {
...
{
    qCDebug(sortingLog) << "Comparing incidence summaries:" << i1->summary() << "with" << i2->summary();
...
{
    qCDebug(sortingLog) << "Comparing incidence summaries (more than):" << i1->summary() << "with" << i2->summary();
    return QString::compare(i1->summary(), i2->summary(), Qt::CaseInsensitive) > 0;
}

bool KCalendarCore::Incidences::categoriesLessThan(const Incidence::Ptr &i1, const Incidence::Ptr &i2)
{
    qCDebug(sortingLog) << "Comparing incidence categories:" << i1->categoriesStr() << "with" << i2->categoriesStr();
...
{
    qCDebug(sortingLog) << "Comparing incidence categories (more than):" << i1->categoriesStr() << "with" << i2->categoriesStr();
    const auto res = QString::compare(i1->categoriesStr(), i2->categoriesStr(), Qt::CaseSensitive);
    if (res == 0) {
        return Incidences::summaryMoreThan(i1, i2);
    } else {
