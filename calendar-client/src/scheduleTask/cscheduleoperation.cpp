...
// Define logging category for schedule operations
Q_LOGGING_CATEGORY(scheduleOperation, "calendar.schedule.operation")
...
CScheduleOperation::CScheduleOperation(const AccountItem::Ptr &accountItem, QWidget *parent)
    ...
    , m_widget(parent)
{
    qCDebug(scheduleOperation) << "Creating schedule operation with account:" << (accountItem ? accountItem->getAccount()->accountID() : "null");
    //如果为空默认设置为本地帐户
    if (m_accountItem.isNull()) {
        qCDebug(scheduleOperation) << "Account is null, setting to local account";
...
CScheduleOperation::CScheduleOperation(const QString &scheduleTypeID, QWidget *parent)
    ...
    , m_widget(parent)
{
    qCDebug(scheduleOperation) << "Creating schedule operation with scheduleTypeID:" << scheduleTypeID;
    //如果为空默认设置为本地帐户
    if (m_accountItem.isNull()) {
        qCWarning(scheduleOperation) << "Cannot get account by schedule type,scheduleTypeID:" << scheduleTypeID;
        m_accountItem = gAccountManager->getLocalAccountItem();
    }
}
...
{
    qCInfo(scheduleOperation) << "Creating schedule with ID:" << scheduleInfo->uid();
    //如果为农历且重复类型为每年
    if (scheduleInfo->lunnar() && scheduleInfo->getRRuleType() == DSchedule::RRule_Year) {
        qCDebug(scheduleOperation) << "Lunar yearly schedule detected, showing lunar message dialog";
        lunarMessageDialogShow(scheduleInfo);
    }
    m_accountItem->createSchedule(scheduleInfo);
    qCInfo(scheduleOperation) << "Schedule created successfully";
    return true;
}
...
{
    qCInfo(scheduleOperation) << "Changing schedule with ID:" << oldInfo->uid();
    bool _result {false};

    if (newInfo->getRRuleType() == DSchedule::RRule_None && newInfo->getRRuleType() == oldInfo->getRRuleType()) {
        qCDebug(scheduleOperation) << "Updating non-recurring schedule";
        m_accountItem->updateSchedule(newInfo);
        _result = true;
    } else {
        qCDebug(scheduleOperation) << "Handling recurring schedule change";
        //如果切换了全天状态则提醒是否修改全部
        if (newInfo->allDay() != oldInfo->allDay()) {
            qCDebug(scheduleOperation) << "All-day status changed, showing confirmation dialog";
            ...
            qCDebug(scheduleOperation) << "Recurrence rule changed, showing confirmation dialog";
            CScheduleCtrlDlg msgBox(m_widget);
    ...
    qCInfo(scheduleOperation) << "Schedule change completed with result:" << _result;
    return  _result;
}
...
{
    qCInfo(scheduleOperation) << "Deleting schedule with ID:" << scheduleInfo->uid();
    bool _restuleBool {false};
    //如果为普通日程
    if (scheduleInfo->getRRuleType() == DSchedule::RRule_None) {
        qCDebug(scheduleOperation) << "Deleting non-recurring schedule";
        CScheduleCtrlDlg msgBox(m_widget);
        ...
        qCDebug(scheduleOperation) << "Handling recurring schedule deletion";
        //获取原始日程
        ...
        if (num == 1) {
            qCDebug(scheduleOperation) << "Handling single occurrence deletion";
            CScheduleCtrlDlg msgBox(m_widget);
            msgBox.setText(tr("You are deleting an event."));
            ...
            qCDebug(scheduleOperation) << "Handling multiple occurrence deletion";
            CScheduleCtrlDlg msgBox(m_widget);
            msgBox.setText(tr("You are deleting an event."));
    ...
    qCInfo(scheduleOperation) << "Schedule deletion completed with result:" << _restuleBool;
    return _restuleBool;
}
...
{
    qCInfo(scheduleOperation) << "Deleting single occurrence of schedule with ID:" << scheduleInfo->uid();
    //如果为纪念日或节日则不处理
    if (isFestival(scheduleInfo)) {
        qCDebug(scheduleOperation) << "Skipping deletion for festival schedule";
        return;
    }
    //如果为普通日程则删除
    ...
    qCInfo(scheduleOperation) << "Single occurrence deletion completed";
}
...
{
    qCDebug(scheduleOperation) << "Changing recurring schedule info for ID:" << oldinfo->uid();
    bool _result{false};
    //如果为重复类型第一个
    // 获取原始数据
    ...
    if (num == 1) {
        qCDebug(scheduleOperation) << "Handling single occurrence change";
        CScheduleCtrlDlg msgBox(m_widget);
        ...
        qCDebug(scheduleOperation) << "Handling multiple occurrence change";
        CScheduleCtrlDlg msgBox(m_widget);
        msgBox.setText(tr("You are changing a repeating event."));
        msgBox.setInformativeText(
    ...
    qCDebug(scheduleOperation) << "Recurring schedule change completed with result:" << _result;
    return _result;
}
...
{
    qCDebug(scheduleOperation) << "Changing single occurrence for schedule ID:" << oldinfo->uid();
    //修改日程
    DSchedule::Ptr newschedule(newinfo->clone());
    newschedule->setRRuleType(DSchedule::RRule_None);
    newschedule->setRecurrenceId(QDateTime());
    ...
    qCDebug(scheduleOperation) << "Single occurrence change completed";
    return true;
}
...
{
    qCDebug(scheduleOperation) << "Changing repetition rule for schedule ID:" << oldinfo->uid();
    ...
    qCDebug(scheduleOperation) << "Repetition rule change completed";
}

void CScheduleOperation::lunarMessageDialogShow(const DSchedule::Ptr &newinfo)
{
    qCDebug(scheduleOperation) << "Showing lunar message dialog for schedule ID:" << newinfo->uid();
...
{
    qCDebug(scheduleOperation) << "Checking if lunar message dialog needed for schedule ID:" << newinfo->uid();
...
{
    qCDebug(scheduleOperation) << "Checking if schedule is festival, ID:" << schedule->uid();
    //判断是否为节假日日程
    AccountItem::Ptr account = gAccountManager->getAccountItemByScheduleTypeId(schedule->scheduleTypeID());
    if (account.isNull()) {
        qCWarning(scheduleOperation) << "Cannot get account by schedule type,scheduleTypeID:" << schedule->scheduleTypeID();
        return false;
    }
...
{
    qCDebug(scheduleOperation) << "Checking if schedule is invariant, ID:" << schedule->uid();
    ...
    if (account->accountType() == DAccount::Account_UnionID) {
        ...
        if (netManger.getNetWorkState() != DOANetWorkDBus::Active
            ...
            qCWarning(scheduleOperation) << "Schedule is invariant due to network state or account settings";
            return true;
        }
    }
    return false;
