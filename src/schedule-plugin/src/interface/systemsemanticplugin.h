...
public:
    ...
    virtual bool needRunInDifferentThread()
    {
        return true; /*当前线程是否需要在单独的线程中运行*/
    }
    virtual bool isAutoDetectSession()
    {
        return true; /*是否自动检测多轮对话*/
    }
    virtual int getPluginProtocolVersion()
    {
        return AIAPluginProtocolVersion;
    }
